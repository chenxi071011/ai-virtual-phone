...
import { AlertCircle, Blocks, Check, Trash2, User, ChevronLeft, ChevronRight, Clapperboard, Clock, Gift, HeartPulse, Languages, Loader2, MoreHorizontal, X } from "lucide-react";
...
export function ChatRoom({ session, onBack }: ChatRoomProps) {
        ...
        for (const r of results) {
            ...
            for (const part of parts) {
                ...
                const draft = buildAssistantMessageDraft(part, {
                    ...
                    reasoning: attachHere && reasoning ? reasoning : undefined,
            ...
            if (!attachedState && (statusPanel || innerMonologue || reasoning || stateValues.length > 0)) {
                throwIfGenerationStopped(guard);
                const msg = pushChatMessage({
                    sessionId: session.id,
                    role: "assistant",
    ...
    const splitAndSaveAIMessages = async (
        ...
        const { parts: rawParts, stateValues, freshStateValues, statusPanel, innerMonologue, reasoning } = parseAIResponse(aiResponseText, previousState);
        ...
        if (metaIdx === -1 && (statusPanel || innerMonologue || reasoning || stateValues.length > 0)) {
        ...
        for (let idx = 0; idx < filteredParts.length; idx += 1) {
            ...
            const draft = buildAssistantMessageDraft(part, {
                ...
                reasoning: idx === metaIdx && reasoning ? reasoning : undefined,
            ...
            if (session.isGroup) {
                ...
                const results = await generateGroupChatCompletion(session, latestMessages, {
                    onTextPart: async (text, senderInfo, options) => {
                        ...
                        const { parts: rawParts, stateValues, freshStateValues, statusPanel, innerMonologue, reasoning } = parseAIResponse(text, previousState);
    ...
    const handleEditResponseSave = () => {
        ...
        if (session.isGroup && editingResponseRoundId) {
            ...
            for (const segment of segments) {
                const responseBatchId = createResponseBatchId();
                const { parts: rawParts, stateValues, freshStateValues, statusPanel, innerMonologue, reasoning } = parseAIResponse(segment.responseText, getCurrentStateForCharacter(segment.characterId));
        ...
        const { parts: rawParts, stateValues, freshStateValues, statusPanel, innerMonologue, reasoning } = parseAIResponse(editedResponseContent, previousState);
        const parts = stripInvalidStickerParts(rawParts);
        ...
        if (metaPartIndex === -1) {
            if (statusPanel || innerMonologue || reasoning || stateValues.length > 0) {
                normalizedParts.push({ content: "" });
    ...
    const projectedMessages = useMemo<RenderChatMessage[]>(() => {
        ...
        for (const msg of dedupedMessages) {
            ...
            const storedMeta = batch.find(m => m.statusPanel || m.innerMonologue || m.reasoning || (m.stateValues && m.stateValues.length > 0));
            ...
            parts.forEach((part, index) => {
                ...
                const reasoningHere = isMetaSlot ? (parsed.reasoning || storedMeta?.reasoning) : undefined;
        ...
        <div ref={wrapperRef} className={`session-${session.id} chat-room-wrapper page-shell inset-0 flex flex-col z-20`} style={chatRoomBackgroundStyle} {...(bgLoading ? { "data-loading": "" } : {})} {...(bgImageResolved ? { "data-has-bg-image": "" } : {})} {...(showSettings ? { "data-settings-open": "" } : {})}>
            ...
            >
                ...
                {!offlineMode && projectedMessages.map((msg, idx) => {
                    ...
                    return (
                        <div key={msg.id} className="flex flex-col gap-4" {...(hiddenEmpty ? { style: { display: "none" } } : {})} {...(isEmptyBubble && renderMsg.reasoning && !showTime ? { "data-reasoning-only": "" } : {})}>
                            {showTime && (
                            ...
                            <div
                                ...
                                {...(isEmptyBubble && renderMsg.reasoning ? { "data-reasoning-empty": "" } : {})}
